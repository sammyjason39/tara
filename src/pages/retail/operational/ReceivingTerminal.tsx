              ...
              <Card className="shadow-2xl border-slate-200 rounded-[2rem] overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50/80 p-8">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const first = shipments.find(
                        (s) => s.status === "pending",
                      );
                      if (first) {
                        startIntake(first);
                        toast({
                          title: "Slip Recognized",
                          description: `Loading Manifest: ${first.id}`,
                        });
                      } else {
                        toast({
                          title: "Scan Error",
                          description:
                            "No pending shipments found in the logistics node.",
                        });
                      }
                    }}
                  >
                    ...
                    {isLoading ? (
                      ...
                      shipments
                        ...
                        .map((shipment) => (
                          <div
                            key={shipment.id}
                            className="p-6 rounded-[1.5rem] border-2 border-slate-50 flex items-center justify-between hover:border-blue-200 hover:shadow-2xl transition-all cursor-pointer group bg-white"
                          >
                            <div className="flex gap-5">
                              ...
                              <div>
                                ...
                                <div className="text-xl font-black italic text-slate-900 leading-tight tracking-tighter">
                                  {shipment.id}
                                </div>
                                <div className="text-[10px] text-blue-600 font-bold uppercase tracking-tighter mt-1">
                                  {shipment.vendor}
                                </div>
                              </div>
                            </div>
                              ...
                              <Badge
                                className={
                                  shipment.priority === "HIGH"
                                    ? "bg-red-50 text-red-600 border-none font-black italic text-[9px] px-3"
                                    : "bg-slate-100 text-slate-500 border-none font-black italic text-[9px] px-3"
                                }
                              >
                      ...
                      shipments.filter((s) => s.status !== "completed")
                        ...
                        <div className="py-20 flex flex-col items-center justify-center text-slate-300">
                          ...
                          <p className="font-black italic uppercase tracking-[0.3em] text-xs">
                            Dock Cleaned • Manifests Complete
                          </p>
                        </div>
                      )}
                  </div>
                            ...
                            <div
                              ...
                              className={`p-6 rounded-2xl border-2 transition-all flex items-center justify-between group ${
                                hasVariance && item.received > 0
                                  ? "bg-red-50/50 border-red-100 shadow-md"
                                  : "bg-white border-slate-50 hover:border-slate-200"
                              }`}
                            >
                              <div className="flex gap-5">
                                <div
                                  className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${hasVariance && item.received > 0 ? "bg-red-100" : "bg-slate-50"}`}
                                >
                                  <Package2
                                    className={`w-7 h-7 ${hasVariance && item.received > 0 ? "text-red-600" : "text-slate-300"}`}
                                  />
                                </div>
                                <div className="min-w-0 flex flex-col justify-center">
                                  <div className="text-base font-black text-slate-900 italic leading-none">
                                    {item.name}
                                  </div>
                              ...
                              <div className="flex items-center gap-16">
                                <div className="text-center">
                                  ...
                                  <div className="text-xl font-black italic text-slate-500">
                                    {item.expected}
                                  </div>
                                </div>
                                <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border-2 border-slate-100 shadow-inner">
                                  <Button
                                    ...
                                    onClick={() =>
                                      updateReceived(item.itemId, -1)
                                    }
                                  >
                                    -
                                  </Button>
                                  <div className="w-16 text-center text-2xl font-black italic tracking-tighter text-slate-900">
                                    {item.received}
                                  </div>
                                  <Button
                                    className="h-10 w-10 p-0 bg-white hover:bg-slate-100 text-slate-400 border border-slate-200 shadow-sm rounded-xl font-black"
          ...
          <div className="space-y-6">
            <Card className="border-red-100 bg-red-50/20 shadow-xl rounded-[2rem] border-2 overflow-hidden">
              ...
              <CardContent className="p-8 space-y-6">
                <p className="text-[11px] text-red-800 font-bold leading-relaxed italic">
                  Critical Alert: <strong>3 SKU mismatches</strong> detected in
                  morning transit. Shipment{" "}
            ...
            <Card className="shadow-2xl border-slate-100 rounded-[2rem] bg-white overflow-hidden">
              <CardHeader className="border-b bg-slate-50/50 p-6">
                <CardTitle className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic">
                  Process Velocity
                </CardTitle>
              </CardHeader>
              <CardContent className="p-8 space-y-8">
                ...
                <div className="space-y-4 pt-2">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black uppercase text-slate-500 tracking-tighter italic">
                      Ledger Sync: REALTIME
                    </span>
                  </div>
                  <div className="flex items-center gap-4">
