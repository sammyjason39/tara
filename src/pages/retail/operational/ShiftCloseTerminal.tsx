...
import { Lock, Banknote, Calculator, AlertCircle, FileCheck, Landmark, ShieldCheck, RefreshCw, FileText, ChevronRight } from "lucide-react";
...
const ShiftCloseTerminal = () => {
  ...
  useEffect(() => {
    const fetchShift = async () => {
      try {
        ...
        const current = shifts.find(s => s.employeeId === session.userId && s.status === "open");
        if (current) {
  ...
  const handleCloseShift = async () => {
    if (!activeShift) {
      toast({ title: "No Open Shift", description: "This terminal does not have an active session.", variant: "destructive" });
      return;
    }
    if (!actualCash) {
       toast({ title: "Input Required", description: "Please enter the physical cash count.", variant: "destructive" });
       return;
    }
  ...
  if (!activeShift && !isClosed) {
    return (
      <div className="max-w-2xl mx-auto py-20 text-center space-y-6">
        ...
        <div>
          <h2 className="text-3xl font-black italic text-slate-900 tracking-tighter uppercase">No Active Shift Detected</h2>
          <p className="text-slate-500 font-medium">Please open a new shift from the Shift Control panel first.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto py-20 animate-in zoom-in-95 duration-500">
         <Card className="border-green-200 bg-green-50/20 shadow-3xl rounded-[2.5rem] overflow-hidden">
            <CardContent className="p-12 text-center space-y-8">
               ...
               <div>
                  <h2 className="text-4xl font-black italic text-slate-900 tracking-tighter uppercase">Reconciled & Locked</h2>
                  <p className="text-slate-500 font-black mt-2 tracking-widest text-[10px] uppercase">Compliance ID: <span className="text-blue-600">ZVX-RECON-{activeShift?.id}</span></p>
               </div>
               ...
               <div className="grid grid-cols-2 gap-6 text-left">
                  <div className="p-6 bg-white rounded-2xl border-2 border-emerald-100 shadow-inner">
                     <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Physical Cash</div>
                     <div className="text-2xl font-black italic tracking-tighter text-slate-900">Rp {parseInt(actualCash).toLocaleString()}</div>
                  </div>
                  <div className="p-6 bg-white rounded-2xl border-2 border-emerald-100 shadow-inner">
                     <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Ledger Variance</div>
                     <div className={`text-2xl font-black italic tracking-tighter ${variance < 0 ? 'text-red-600' : variance > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
               ...
               <Button 
                ...
                onClick={() => {
                  toast({ title: "Archive Generated", description: "Secure PDF summary has been dispatched to Zenvix Compliance Vault." });
      ...
      <WorkspacePanel className="flex-1 overflow-auto rounded-[2rem]">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <Card className="border-blue-200 bg-gradient-to-br from-blue-50/50 to-indigo-50/50 shadow-2xl rounded-[2rem] relative overflow-hidden group">
              ...
              <CardContent className="px-8 pb-8">
                <div className="text-5xl font-black text-blue-900 tracking-tighter italic">Rp {EXPECTED_CASH.toLocaleString()}</div>
                <p className="text-[10px] text-blue-600 mt-2 uppercase font-black italic tracking-widest opacity-60">Aggregated from Local POS Vault</p>
              </CardContent>
            </Card>
            ...
            <Card className="border-slate-200 shadow-2xl rounded-[2rem] bg-white group overflow-hidden">
              <CardHeader className="pb-4 p-8">
                <CardTitle className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none flex items-center gap-3 italic">
                  <ChevronRight className="w-4 h-4 text-indigo-500" /> Digital Settlements
                </CardTitle>
              </CardHeader>
              <CardContent className="px-8 pb-8">
                <div className="text-5xl font-black text-slate-900 tracking-tighter italic">Rp {EXPECTED_CARD.toLocaleString()}</div>
                <p className="text-[10px] text-slate-500 mt-2 uppercase font-black italic tracking-widest opacity-60">Verified via PCI-DSS Gateway</p>
              </CardContent>
            </Card>
        </div>
        ...
        <Card className="mb-8 shadow-3xl border-slate-100 overflow-hidden rounded-[2.5rem]">
          ...
          <CardContent className="p-10 space-y-10">
            <div className="flex flex-col items-center justify-center p-12 bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-200 relative group transition-all hover:bg-white hover:border-blue-200">
               <div className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 italic">Input Total Physical Cash (IDR)</div>
               <div className="relative w-96">
                 <span className="absolute left-6 top-1/2 -translate-y-1/2 text-5xl font-black text-slate-200 italic tracking-tighter transition-colors group-focus-within:text-blue-200">Rp</span>
                 <Input 
            ...
            {actualCash && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-5 duration-500">
                <div className={`p-8 rounded-[2rem] border-2 flex flex-col justify-center transition-all ${needsExplanation ? 'bg-red-50 border-red-100 shadow-xl shadow-red-500/10' : 'bg-emerald-50 border-emerald-100 shadow-xl shadow-emerald-500/10'}`}>
                   <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 italic">Calculated Discrepancy</div>
                   <div className={`text-4xl font-black italic tracking-tighter ${variance < 0 ? 'text-red-700' : variance > 0 ? 'text-amber-700' : 'text-emerald-700'}`}>
                     Rp {variance.toLocaleString()}
                   </div>
                   <p className="text-[10px] font-black text-slate-500 mt-2 uppercase tracking-widest">{variance === 0 ? 'Ledger Balanced' : 'RECONCILIATION REQUIRED'}</p>
                </div>

                {needsExplanation && (
                  <div className="space-y-4">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-red-600 flex items-center gap-3 italic">
                      <AlertCircle className="w-5 h-5 animate-pulse" /> Compliance Note Mandatory
                    </label>
                    <Textarea 
        ...
        <div className="flex flex-col items-center gap-8">
          <Button 
            ...
            disabled={isClosing}
          >
            {isClosing ? <RefreshCw className="w-10 h-10 animate-spin" /> : (
              <>
          ...
          <div className="flex items-center gap-4 px-10 py-4 bg-slate-100 rounded-full border border-slate-200 shadow-inner">
             ...
             <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.15em] italic">
              Digital Signature will be appended to the <span className="text-slate-900">Zenvix Fiscal Archive</span>
             </p>
          </div>
        </div>
      </WorkspacePanel>
