...
import { 
  Scan, ShoppingBag, CreditCard, QrCode, ArrowRight, Trash2, 
  Minus, Plus, Info, ShieldCheck, Monitor, Sparkles, X, CheckCircle, RefreshCw 
} from "lucide-react";
...
const SelfServiceKiosk = () => {
  ...
  const [step, setStep] = useState<"scanning" | "payment" | "success">("scanning");
  ...
  const addToCart = (product: any) => {
    setCart(prev => {
      ...
      return [...prev, { id: product.id, sku: product.sku, name: product.name, price: product.price, quantity: 1 }];
    });
    toast({ title: "Item Added", description: `${product.name} added to tray.` });
  };
  ...
  const handleScan = (e: React.FormEvent) => {
    ...
    if (product) {
      ...
      toast({ title: "Scan Error", description: "Product unknown. Please try again.", variant: "destructive" });
    }
    setScanInput("");
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      await retailService.processPayment(session.tenantId!, session, order.id, total, paymentMethod || "card");
    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center space-y-8 animate-in zoom-in-95 duration-500">
        ...
        <div className="text-center space-y-4">
          <h1 className="text-5xl font-black italic tracking-tighter text-slate-900 uppercase">Payment Received!</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">Please take your receipt and items below.</p>
        </div>
        <Button 
      ...
      <WorkspacePanel className="flex-1 overflow-auto rounded-[2rem]">
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100 mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black italic tracking-tighter text-slate-900 uppercase">Nexus Kiosk V2</h1>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-0.5 italic">Station ID: JKT-KIOSK-001 • Secured by Zenvix</p>
            </div>
          </div>
          <div className="flex items-center gap-6">
             <div className="text-right">
                <div className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-0.5 italic">System Integrity</div>
                <div className="flex items-center gap-1.5 justify-end">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                  <span className="text-[10px] font-black italic uppercase">PCI_DSS: ACTIVE</span>
                </div>
             </div>
             <Button 
              variant="ghost" 
              ...
              onClick={() => toast({ title: "Kiosk Support", description: "Terminal ID: JKT-KIOSK-001. Please see the floor supervisor for manual assistance." })}
             >
        ...
        <div className="lg:col-span-8 space-y-8">
              ...
              <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
                 {products.slice(0, 4).map(p => (
                   ...
                   >
                     ...
                     <div className="text-xs font-black italic text-slate-900 truncate uppercase">{p.name}</div>
                     <div className="text-[10px] font-bold text-blue-600 mt-1 italic">Rp {p.price.toLocaleString()}</div>
                   </button>
                 ))}
              </div>
            </CardContent>
          ...
          <Card className="border-none shadow-2xl bg-white rounded-[3rem] overflow-hidden">
             <CardHeader className="p-10 border-b border-slate-50 bg-slate-50/20">
                <CardTitle className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic">Your Selection Vault</CardTitle>
             </CardHeader>
                ...
                <ScrollArea className="h-[400px]">
                   {cart.length === 0 ? (
                     <div className="h-full flex flex-col items-center justify-center py-20 text-slate-200">
                        ...
                        <p className="text-sm font-black italic uppercase tracking-[0.2em]">Tray is Empty</p>
                     </div>
                   ) : (
                          ...
                          <div key={item.id} className="p-8 flex items-center justify-between group hover:bg-slate-50 transition-colors">
                             <div className="flex gap-6">
                                ...
                                <div className="flex flex-col justify-center">
                                   <div className="text-xl font-black italic text-slate-900 uppercase tracking-tighter">{item.name}</div>
                                   <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1 italic">SKU: {item.sku}</div>
                                </div>
                             </div>
                             <div className="flex items-center gap-12">
                                <div className="flex items-center gap-4 bg-white border-2 border-slate-100 p-2 rounded-2xl shadow-sm">
                                   <Button 
                                    variant="ghost" className="h-10 w-10 p-0 rounded-xl hover:bg-slate-100 font-black"
                                    onClick={() => setCart(prev => prev.map(i => i.id === item.id ? { ...i, quantity: Math.max(1, i.quantity - 1) } : i))}
                                   >
                                     <Minus className="w-5 h-5" />
                                   </Button>
                                   <div className="w-10 text-center text-xl font-black italic text-slate-900">{item.quantity}</div>
                                   <Button 
                                    variant="ghost" className="h-10 w-10 p-0 rounded-xl hover:bg-slate-100 font-black"
                                    onClick={() => addToCart({ id: item.id, name: item.name, sku: item.sku, price: item.price })}
                                   >
                                     <Plus className="w-5 h-5" />
                                   </Button>
                                </div>
                                <div className="text-right min-w-[120px]">
                                   <div className="text-2xl font-black italic text-slate-900 tracking-tighter">Rp {(item.price * item.quantity).toLocaleString()}</div>
                                   <button 
                                    onClick={() => {
                                      if (confirm("Clear all items from your tray?")) {
                                        setCart([]);
                                        toast({ title: "Tray Cleared", description: "All items have been removed." });
                                      }
                                    }}
        ...
        <div className="lg:col-span-4 space-y-8">
           <Card className="border-none shadow-3xl bg-slate-900 text-white rounded-[3rem] overflow-hidden sticky top-8">
              <CardContent className="p-10 space-y-10">
                 <div className="space-y-4">
                    <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em] italic">Consolidated Total</div>
                    ...
                    <div className="flex items-center gap-3 py-4 border-y border-white/10">
                       ...
                       <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Encrypted Transaction Context</span>
                    </div>
                 </div>
                 ...
                 {step === "scanning" ? (
                   <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                         <button 
                          ...
                          className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${
                            paymentMethod === "card" ? "bg-blue-600 border-blue-400 shadow-2xl shadow-blue-500/50" : "bg-white/5 border-white/10 hover:bg-white/10"
                          }`}
                         >
                            <CreditCard className="w-8 h-8" />
                            <span className="text-[10px] font-black uppercase tracking-widest italic">Credit Card</span>
                         </button>
                         <button 
                          onClick={() => setPaymentMethod("qr")}
                          className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${
                            paymentMethod === "qr" ? "bg-amber-600 border-amber-400 shadow-2xl shadow-amber-500/50" : "bg-white/5 border-white/10 hover:bg-white/10"
                          }`}
                         >
                            <QrCode className="w-8 h-8" />
                            <span className="text-[10px] font-black uppercase tracking-widest italic">QR / E-Wallet</span>
                         </button>
                      </div>
                      ...
                      >
                        {isProcessing ? <RefreshCw className="w-8 h-8 animate-spin" /> : (
                          <>Pay Now <ArrowRight className="w-8 h-8 ml-4 shrink-0" /></>
                        )}
                      </Button>
                   </div>
                 ) : (
                   <div className="space-y-6 animate-pulse">
                      <p className="text-center font-black italic text-sm text-blue-400 uppercase tracking-widest">Awaiting Nexus Verification...</p>
                 ...
                 <div className="text-center">
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">
                       By paying, you agree to our <span className="underline text-slate-400">Digital Terms of Service</span>. 
           ...
           <Card className="border-2 border-slate-100 shadow-xl bg-white rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-8 flex gap-5 items-center">
                 ...
                 <div>
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic leading-none mb-1">Station Telemetry</div>
                    <div className="text-sm font-black italic text-slate-900 uppercase tracking-tighter">Thermal Printer: [OK]</div>
                 </div>
              </CardContent>
           </Card>
        </div>
