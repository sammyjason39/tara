...
import { ShoppingCart, Search, CreditCard, Banknote, User, Package, Trash2, Plus, Minus, CheckCircle2, RefreshCw } from "lucide-react";
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredProducts.map((p) => (
                  ...
                  >
                    ...
                    <CardContent className="p-4">
                      <div className="text-sm font-black text-slate-900 truncate italic">{p.name}</div>
                      <div className="text-[10px] text-slate-400 font-bold mb-2 uppercase tracking-tighter">SKU: {p.sku}</div>
                      <div className="text-base font-black text-blue-600">Rp 85,000</div>
                    </CardContent>
                  </Card>
                ))}
              </div>
        ...
        <div className="flex-1 flex flex-col space-y-4 overflow-hidden">
          <Card className="flex-1 flex flex-col overflow-hidden shadow-2xl border-slate-200 rounded-[2.5rem]">
            <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50/80 py-6 px-8">
              ...
              <Badge className="bg-blue-600 text-white font-black italic px-4 py-1">{cart.length} SKUs</Badge>
            </CardHeader>
            <CardContent className="flex-1 p-0 overflow-hidden flex flex-col bg-white">
              <ScrollArea className="flex-1 p-6">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-300 py-20">
                    ...
                    <p className="font-black italic uppercase text-xs tracking-widest text-slate-400">Cart is Vacant</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {cart.map((item) => (
                      <div key={item.id} className="flex gap-4 hover:bg-slate-50 p-3 rounded-2xl transition-colors group relative border border-transparent hover:border-slate-100 italic">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[10px] text-slate-500 font-bold uppercase">Rp {item.price.toLocaleString()} / Unit</div>
                          <div className="flex items-center gap-4 mt-3">
                            <div className="flex items-center border-2 border-slate-100 rounded-xl bg-white overflow-hidden shadow-sm">
                              ...
                              <span className="px-4 text-xs font-black min-w-[2.5rem] text-center text-slate-900">{item.quantity}</span>
                              <button 
                                className="p-2 hover:bg-slate-50 text-slate-400 transition-colors active:bg-slate-100"
                                onClick={() => updateQuantity(item.id, 1)}
                              >
                        ...
                        <div className="text-sm font-black text-right text-slate-900 pt-1">Rp {(item.price * item.quantity).toLocaleString()}</div>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="p-8 bg-slate-900 text-white space-y-6 rounded-t-[3rem] shadow-3xl">
                <div className="space-y-4 text-sm font-bold italic opacity-80">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400 uppercase tracking-widest text-[10px] font-black">VAT (11%)</span>
                    <span className="text-white">Rp {tax.toLocaleString()}</span>
                  </div>
                </div>
                <Separator className="bg-white/10" />
                <div className="flex justify-between items-end pb-2">
                  <span className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] mb-1">Due Amount</span>
                  <span className="text-4xl font-black italic tracking-tighter">Rp {total.toLocaleString()}</span>
                </div>
          ...
          <Card className="hover:border-blue-200 transition-colors shadow-xl rounded-3xl overflow-hidden border-2 border-slate-100">
            <CardContent className="p-1">
              <Button variant="ghost" className="w-full justify-start gap-4 text-slate-600 font-black italic h-14 hover:bg-blue-50 hover:text-blue-600 transition-all group px-5">
