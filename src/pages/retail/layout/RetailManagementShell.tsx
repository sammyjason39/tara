...
import { 
  ...
  PackageCheck, Tag, Eye, MonitorDot, FileText, Layout
} from "lucide-react";
...
const SECTIONS: MenuSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Commerce Channels", to: "/m/retail/management/ecommerce", icon: Globe },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Device Control", to: "/m/retail/management/devices", icon: MonitorDot },
      { label: "Audit Ledger", to: "/m/retail/management/audit", icon: FileText },
    ],
  },
];
...
const ROUTE_LABELS: Record<string, string> = Object.fromEntries(
  SECTIONS.flatMap((section) =>
    section.items.map((item) => [item.to.replace("/m/retail/", ""), item.label]),
  ),
);

export const RetailManagementShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const session = useSession();
  const { activeStore } = useRetail();
  const location = useLocation();

  const segments = location.pathname.replace("/m/retail", "").split("/").filter(Boolean);
            ...
            <div className="space-y-6 p-4">
              <WorkspacePanel>
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  <p className="font-bold uppercase tracking-tighter text-[10px]">Context: {session.tenantId}</p>
                </div>
              </WorkspacePanel>
