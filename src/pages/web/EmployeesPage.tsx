...
export function EmployeesPage() {
  ...
  const [newEmp, setNewEmp] = useState({ full_name: "", email: "", department: "", role: "" });
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      ...
      setNewEmp({ full_name: "", email: "", department: "", role: "" });
    } catch (err: any) {
                    ...
                    <div className="relative" ref={openMenuId === emp.id ? menuRef : undefined}>
                      ...
                      {openMenuId === emp.id && (
                        <div className="absolute top-full right-0 mt-1 w-40 rounded-md border border-border bg-card shadow-luxury-lg py-1 z-50 animate-fade-in">
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); setOpenMenuId(null); toast("Fitur edit segera hadir"); }}
                            className="w-full text-left px-3 py-2 text-sm hover:bg-accent transition-colors"
                          >
                            {t("common.edit")}
                          </button>
