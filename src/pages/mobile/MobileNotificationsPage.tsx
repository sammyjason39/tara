  ...
  return (
    <div className="px-5 py-6 space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        {unread > 0 && (
          <button onClick={markAllRead} className="text-xs text-gold hover:text-gold/80">Tandai semua dibaca</button>
        )}
      </div>
      ...
      {isLoading ? (
        Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="surface-elevated p-4 animate-pulse">
            <div className="flex gap-3"><div className="h-9 w-9 rounded-full bg-muted shrink-0" /><div className="flex-1 space-y-2"><div className="h-3.5 w-32 bg-muted rounded" /><div className="h-3 w-full bg-muted/50 rounded" /></div></div>
          </div>
        ))
        ...
        <div className="space-y-2">
          {notifications.map((n: any) => (
            <button key={n.id} onClick={() => setExpandedId(expandedId === n.id ? null : n.id)}
              className={cn("w-full text-left surface-elevated p-4 transition-all", !n.is_read && "border-l-2 border-l-gold")}>
              <div className="flex items-start gap-3">
                <div className={cn("h-9 w-9 rounded-full flex items-center justify-center shrink-0",
                  ...
                  {n.visibility === "public" ? <Megaphone className="h-4 w-4 text-info" /> :
                   n.notification_type?.includes("warning") ? <Lock className="h-4 w-4 text-destructive" /> :
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {expandedId === n.id && (
                    <div className="mt-2 pt-2 border-t border-border/50 animate-fade-in">
                      ...
                      <p className="text-2xs text-muted-foreground mt-1">Tipe: {n.notification_type || "—"}</p>
                    </div>
                  )}
                </div>
              </div>
