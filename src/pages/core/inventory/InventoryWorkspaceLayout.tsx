...
const SECTIONS: MenuSection[] = [
  {
    title: "Intelligence",
    items: [
      { label: "Inventory Command", to: "/core/inventory/dashboard", icon: Boxes },
      { label: "Flow Analysis", to: "/core/inventory/insights", icon: BarChart3 },
    ],
  },
  {
    title: "Operations",
    items: [
      ...
      { label: "Warehouse Matrix", to: "/core/inventory/warehouse", icon: Warehouse },
      { label: "Receiving Dock", to: "/core/inventory/receiving", icon: ActivitySquare },
      { label: "Adjustments", to: "/core/inventory/adjustments", icon: ClipboardCheck },
      { label: "IoT Event Feed", to: "/core/inventory/iot-feed", icon: Radio },
    ],
  },
  {
    title: "Engagement",
    items: [
      { label: "Logistics Schedule", to: "/core/inventory/schedule", icon: Users },
    ],
  },
  {
    title: "Governance",
    items: [
      ...
      { label: "Audit Ledger", to: "/core/inventory/audit", icon: History },
    ],
  },
];
...
export default function InventoryWorkspaceLayout() {
  ...
  const segments = location.pathname.replace("/core/inventory", "").split("/").filter(Boolean);
  const breadcrumbs = segments.map((segment, index) => ({
    label: ROUTE_LABELS[segment] ?? segment.replace(/-/g, " "),
    path: `/core/inventory/${segments.slice(0, index + 1).join("/")}`,
  }));
          ...
          <div className="space-y-6 px-4 py-4">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbLink asChild>
                    <Link to="/core/inventory" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-all">INVENTORY ENGINE</Link>
                  </BreadcrumbLink>
                </BreadcrumbItem>
            ...
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                     <h2 className="text-3xl font-black tracking-tighter text-slate-900 dark:text-white uppercase italic leading-none">Inventory Command</h2>
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] leading-relaxed italic mt-1.5 flex items-center gap-2">
                        <Target className="h-3.5 w-3.5 text-blue-500" /> Precision Logistics & Fulfillment Matrix
                     </p>
                  </div>
               </div>
               ...
               <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Inventory Verified</span>
                  </div>
               ...
               <div className="flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 group hover:shadow-xl transition-all duration-500">
                  <div className="flex items-center gap-4">
                     ...
                     <div className="space-y-0.5">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-white leading-none">Stock Telemetry</p>
                        <p className="text-[8px] font-black uppercase tracking-widest text-blue-500 animate-pulse italic leading-none">Synced & Secure</p>
                     </div>
                  </div>
                  <ActivitySquare className="h-5 w-5 text-slate-300 group-hover:text-blue-600 transition-colors" />
               </div>
            </div>
          </div>
        }
