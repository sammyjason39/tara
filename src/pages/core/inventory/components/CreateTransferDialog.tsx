      ...
      <DialogContent className="max-w-3xl max-h-[90vh] bg-muted border-border rounded-[2.5rem] p-0 overflow-hidden shadow-2xl flex flex-col">
          ...
          <div className="space-y-4">
            <div className="relative group">
              <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none">
                <Barcode className="h-6 w-6 text-primary group-focus-within:animate-pulse" />
              </div>
              ...
              {!fromLocation && (
                <p className="text-[10px] font-bold text-warning italic mt-2 ml-4">
                ...
                selectedItems.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-black uppercase tracking-tighter text-muted-foreground truncate leading-none">
                        {item.item.name}
                      </p>
                      <p className="text-[10px] font-mono font-bold text-primary mt-1 uppercase">
                        {item.item.sku}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 bg-muted p-1.5 rounded-xl border border-border">
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-8 w-8 rounded-lg hover:bg-destructive hover:text-destructive"
                      ...
                      <div className="w-10 text-center">
                        ...
                        <p className="text-[8px] font-bold text-muted-foreground uppercase">Max: {item.maxQuantity}</p>
                      </div>
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-8 w-8 rounded-lg hover:bg-success hover:text-success"
                        onClick={() => updateItemQuantity(item.id, item.quantity + 1)}
                      >
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-10 w-10 rounded-xl hover:bg-destructive hover:text-white transition-colors"
        ...
        <DialogFooter className="shrink-0 p-8 bg-muted border-t border-border gap-4 flex flex-row items-center">
          ...
          <Button
            ...
            className="flex-1 h-14 rounded-2xl bg-primary hover:bg-primary text-white font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl shadow-indigo-500/20 group transition-all transform hover:scale-[1.01] active:scale-[0.99] m-0"
          >
            {loading ? (
