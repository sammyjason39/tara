...
export default function ProcurementWorkspaceLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans selection:bg-amber-500/30 overflow-hidden flex flex-col">
      <PageShell
        header={
          <div className="space-y-6 px-4 py-4">
              ...
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink asChild>
                    <Link to="/core" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-amber-600 transition-all flex items-center gap-2">
                ...
                <BreadcrumbItem>
                  <BreadcrumbLink asChild>
                    <Link to="/core/procurement" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-amber-600 transition-all">PROCUREMENT ENGINE</Link>
                  </BreadcrumbLink>
                </BreadcrumbItem>
                  ...
                  <React.Fragment key={item.path}>
                    ...
                    <BreadcrumbItem>
                      {index === breadcrumbs.length - 1 ? (
                        <BreadcrumbPage className="text-[10px] font-black uppercase tracking-widest text-amber-600 italic">{item.label}</BreadcrumbPage>
                      ) : (
                        <BreadcrumbLink asChild>
                          <Link to={item.path} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-amber-600 transition-all">
                            {item.label}
                          </Link>
                        </BreadcrumbLink>
                      )}
            ...
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-6">
                  <div className="h-14 w-14 bg-amber-600 rounded-[1.5rem] flex items-center justify-center shadow-2xl shadow-amber-600/20 group hover:rotate-12 transition-transform duration-500">
                     <ShoppingCart className="h-8 w-8 text-white" />
                  </div>
                  <div>
                     <h2 className="text-3xl font-black tracking-tighter text-slate-900 dark:text-white uppercase italic leading-none">Procurement Command</h2>
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] leading-relaxed italic mt-1.5 flex items-center gap-2">
                        <Target className="h-3.5 w-3.5 text-amber-500" /> Strategic Sourcing & Supply Chain Integrity Matrix
                     </p>
                  </div>
               </div>
               ...
               <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Sourcing Node Verified</span>
                  </div>
              ...
              <div className="p-8 space-y-12">
                ...
                {SECTIONS.map((section) => (
                  <div key={section.title} className="space-y-6">
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 pl-4 border-l-2 border-amber-600/20">
                      {section.title}
                    </p>
                    <div className="space-y-1">
                      {section.items.map((item) => {
                        const Icon = item.icon;
                        const isActive = location.pathname === item.to || (item.to === "/core/procurement" && location.pathname === "/core/procurement/");
                        return (
                          <NavLink
                            key={item.to}
                            to={item.to}
                            className={cn(
                                ...
                                isActive
                                  ? "bg-white dark:bg-slate-800 text-amber-600 shadow-xl shadow-amber-500/10 translate-x-3 border-l-4 border-l-amber-600"
                                  : "text-slate-400 hover:bg-white/50 dark:hover:bg-slate-800/50 hover:text-amber-500 hover:translate-x-2",
                              )
                            }
                          >
               ...
               <div className="flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 group hover:shadow-xl transition-all duration-500">
                  <div className="flex items-center gap-4">
                     <div className="relative">
                        <div className="h-3 w-3 rounded-full bg-amber-500 animate-ping absolute inset-0 opacity-40" />
                        <div className="h-3 w-3 rounded-full bg-amber-500 relative z-10 shadow-[0_0_12px_rgba(245,158,11,0.5)]" />
                     </div>
                     <div className="space-y-0.5">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-white leading-none">Supply Telemetry</p>
                        <p className="text-[8px] font-black uppercase tracking-widest text-amber-500 animate-pulse italic leading-none">Synced & Secure</p>
                     </div>
                  </div>
                  <Truck className="h-5 w-5 text-slate-300 group-hover:text-amber-600 transition-colors" />
               </div>
            </div>
          </div>
        }
