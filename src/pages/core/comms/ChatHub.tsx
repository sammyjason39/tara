...
export default function ChatHub() {
  ...
  const mainContent = (
    <div className="flex h-[calc(100vh-140px)] overflow-hidden bg-slate-50/50 dark:bg-slate-950/20 rounded-[2.5rem] border border-slate-200/50 dark:border-slate-800/50 m-6">
      {/* Sidebar - Channels */}
      <div className="w-80 border-r border-slate-200/50 dark:border-slate-800/50 flex flex-col bg-white dark:bg-slate-900/40 backdrop-blur-xl">
        <div className="p-6 space-y-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
            <Input
              placeholder="Filter transmissions..."
              className="pl-9 h-11 bg-slate-50 dark:bg-slate-800/50 border-none rounded-xl text-xs font-bold"
              value={search}
          ...
          <div className="space-y-1 pb-6">
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 animate-pulse mx-3" />
              ))
            ) : channels.length === 0 ? (
              ...
              channels
                ...
                .map((channel) => (
                  <div
                    ...
                    className={`p-4 rounded-2xl cursor-pointer transition-all flex items-center gap-4 group ${activeChannel?.id === channel.id ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        ...
                        {channel.unreadCount ? (
                          <Badge className="h-4 min-w-[1rem] p-0 flex items-center justify-center bg-rose-500 text-[9px] font-black rounded-full border-none">
                            {channel.unreadCount}
                          </Badge>
                        ) : (
                      ...
                      <p className={`text-[10px] truncate font-medium ${activeChannel?.id === channel.id ? 'text-white/70' : 'text-slate-500'}`}>
      ...
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col bg-white dark:bg-slate-900/20 backdrop-blur-sm relative">
        {!activeChannel ? (
          ...
          <>
            {/* Chat Header */}
            <div className="p-6 border-b border-slate-200/50 dark:border-slate-800/50 flex justify-between items-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-md sticky top-0 z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[9px] font-black text-emerald-500 uppercase tracking-tighter">Transmission Stable</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="icon" className="rounded-xl h-10 w-10 text-slate-400 hover:text-slate-900"><MoreVertical className="h-4 w-4" /></Button>
              </div>
            </div>
                  ...
                  messages.map((msg, i) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex gap-4 group ${isMe ? 'flex-row-reverse' : 'flex-row'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                        <div className={`h-10 w-10 rounded-xl shrink-0 flex items-center justify-center font-black text-xs ${isMe ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'}`}>
                          {msg.senderId[0].toUpperCase()}
                        </div>
                        <div className={`max-w-[70%] space-y-2 ${isMe ? 'items-end' : 'items-start'}`}>
                          <div className={`flex items-center gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                            <span className="text-[10px] font-black uppercase tracking-tighter text-slate-400">@{msg.senderId.split("-")[0]}</span>
                            <span className="text-[9px] font-bold text-slate-300">{new Date(msg.createdAt).toLocaleTimeString()}</span>
                          </div>
                          <div className={`p-4 rounded-2xl text-sm font-medium leading-relaxed shadow-sm ${isMe ? 'bg-primary text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-tl-none'}`}>
                            {msg.body}
                          </div>
                        </div>
                      </div>
            ...
            {/* Input */}
            <div className="p-8 border-t border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md">
              <div className="max-w-4xl mx-auto flex gap-4">
                <div className="relative flex-1 group">
                  <Input
                    placeholder="Enter transmission content..."
                    className="h-14 bg-slate-50 dark:bg-slate-850 border-none rounded-2xl px-6 text-sm font-bold shadow-inner focus-visible:ring-1 focus-visible:ring-primary/20 pr-16"
                    value={newMessage}
                  ...
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent className="bg-slate-900 text-white border-none font-black text-[9px] uppercase tracking-widest px-3 py-1.5 rounded-lg shadow-2xl">Execute Transmission</TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
                </div>
