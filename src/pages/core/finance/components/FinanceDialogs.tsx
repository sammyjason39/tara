        ...
        <div className="grid md:grid-cols-[1fr_2fr]">
            ...
            <div>
              ...
              <div className="mt-8 space-y-3">
                <div className="bg-white/5 border border-white/10 p-4 rounded-2xl">
                  <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1.5">Source Account</p>
                ...
                <div className="bg-white/5 border border-white/10 p-4 rounded-2xl">
                  <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1.5">Authorization</p>
                  <p className="font-bold text-white text-sm flex items-center gap-2">
                    {isHighLevel
                      ? <><CheckCircle2 className="w-4 h-4 text-emerald-400" /> Direct Auth</>
                      : <><Info className="w-4 h-4 text-indigo-400" /> Requires HOD Sign-off</>}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="p-10 bg-white">
            <div className="space-y-6">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Payment Amount (IDR)</label>
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Payment Method</label>
                ...
                <div>
                  <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Funding Source</label>
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Beneficiary</label>
                  <Input value={destination} onChange={(e) => setDestination(e.target.value)}
                    placeholder="Name / Account No." className="h-12 rounded-2xl font-bold border-slate-200" />
                </div>
                <div>
                  <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Requesting Dept</label>
                  <Input value={department} onChange={(e) => setDepartment(e.target.value)}
                    placeholder="e.g. IT, HR, OPS" className="h-12 rounded-2xl font-bold border-slate-200" />
                </div>
              </div>
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Purpose of Payment</label>
              ...
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2.5 block">Extra Metadata (JSON)</label>
                <Textarea value={extraInfo} onChange={(e) => setExtraInfo(e.target.value)}
                  placeholder='{"invoiceId": "INV-1234"}' rows={2}
                  className="font-mono text-xs resize-none rounded-2xl p-4 border-slate-200 bg-slate-50" />
              </div>
      ...
      <DialogContent className="max-w-md rounded-3xl p-8">
        ...
        <div className="space-y-6 pt-4">
          <div className="grid grid-cols-2 text-sm gap-y-4">
            <span className="text-slate-400 font-semibold">Flow ID</span>
            <span className="font-mono font-bold bg-slate-50 px-2 py-1 rounded-lg text-xs">{workflow?.id}</span>
            <span className="text-slate-400 font-semibold">Entity</span>
            <span className="font-bold text-slate-900">{workflow?.entityType} | {workflow?.entityId}</span>
            <span className="text-slate-400 font-semibold">Maker Dept</span>
            <span className="font-medium text-slate-700">{workflow?.makerDept}</span>
            <span className="text-slate-400 font-semibold">Requested By</span>
            <span className="font-medium text-slate-700">{workflow?.requestedBy}</span>
            <span className="text-slate-400 font-semibold">Status</span>
            <ApprovalStatusBadge status={workflow?.status || "PENDING"} />
          </div>
          <div className="border-t border-slate-100 pt-5">
            <p className="text-xs font-medium leading-relaxed text-slate-400 bg-slate-50 p-4 rounded-2xl">
              Automatically routed based on departmental thresholds. Verify supporting documentation if required.
            </p>
            {workflow?.status === "PENDING" && (
              <div className="mt-4 flex gap-3">
                <Button className="flex-1 h-12 rounded-2xl bg-emerald-500 hover:bg-emerald-600 font-black shadow-lg shadow-emerald-500/20"
                  onClick={() => { onApprove(workflow.id); onClose(); }}>
                  Approve
                </Button>
                <Button className="flex-1 h-12 rounded-2xl border border-rose-200 bg-rose-50 text-rose-500 hover:bg-rose-100 font-black" variant="outline"
      ...
      <DialogContent className="max-w-md rounded-3xl p-8">
        ...
        <div className="space-y-6 pt-4">
          <div className="rounded-2xl bg-rose-50 border border-rose-100 p-5 text-rose-900">
            ...
            <p className="text-sm font-medium text-rose-700/80">{alert?.description}</p>
          </div>
          <div className="grid grid-cols-2 text-sm gap-y-4">
            <span className="text-slate-400 font-semibold">Severity</span>
            <span className="font-black uppercase text-slate-900">{alert?.severity}</span>
