...
export class SalesController {
  ...
  async getDashboard(@Req() request: RequestWithTenant) {
    ...
    return { success: true, tenantId, data: await this.salesService.getDashboard(tenantId) };
  }
  ...
  @Post('leads')
  async createLead(@Req() request: RequestWithTenant, @Body() dto: CreateLeadDto) {
    const { tenantId } = request.tenantContext;
    return {
      success: true,
      tenantId,
  ...
  async convertLead(@Req() request: RequestWithTenant, @Param('id') leadId: string) {
    const { tenantId } = request.tenantContext;
    return {
      success: true,
      tenantId,
  ...
  async moveOpportunityStage(
    ...
    return {
      ...
      data: await this.salesService.moveOpportunityStage(tenantId, opportunityId, dto),
    };
  }
  ...
  async closeOpportunity(
    ...
    return {
      ...
      data: await this.salesService.closeOpportunity(tenantId, opportunityId, dto),
    };
  }
  ...
  @Post('quotes')
  async createQuote(@Req() request: RequestWithTenant, @Body() dto: CreateQuoteDto) {
    const { tenantId } = request.tenantContext;
    return {
      success: true,
      tenantId,
  ...
  async submitQuote(@Req() request: RequestWithTenant, @Param('id') quoteId: string) {
    const { tenantId } = request.tenantContext;
    return {
      success: true,
      tenantId,
  ...
  @Post('tasks')
  async createTask(@Req() request: RequestWithTenant, @Body() dto: CreateTaskDto) {
    const { tenantId } = request.tenantContext;
    return {
      success: true,
      tenantId,
  ...
  @Put('tasks/:id/done')
  async completeTask(@Req() request: RequestWithTenant, @Param('id') taskId: string) {
    const { tenantId } = request.tenantContext;
    return {
      success: true,
      tenantId,
