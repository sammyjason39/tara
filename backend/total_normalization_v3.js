...
for (const snake of sortedSnakes) {
    ...
    // Line definition: INDENT snake SPACE TypePossiblyOptional REST
    const fieldDefRegex = new RegExp(`^(\\s+)${snake}(\\s+)([A-Z][a-zA-Z\\[\\]]+)(\\?)?(.*)$`, 'gm');
    processedContent = processedContent.replace(fieldDefRegex, (match, indent, space, type, optional, rest) => {
        if (rest.includes('@map')) return match;
        const optStr = optional || '';
        return `${indent}${camel}${space}${type}${optStr}${rest} @map("${snake}")`;
    });
...
// 3. FINAL SYNTAX CLEANUP: Ensure ? is before @map
processedContent = processedContent.replace(/(@map\("[^"]+"\))\?/g, '? $1');

fs.writeFileSync(schemaPath, processedContent);
console.log('Total System Normalization V3 Complete.');
