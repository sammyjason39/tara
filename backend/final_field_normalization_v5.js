...
for (let line of lines) {
    ...
    // Identify and normalize field definition
    ...
    // Update constraints using identified fields for CURRENT model
    if (line.includes('@@unique') || line.includes('@@index') || line.includes('@relation')) {
        for (const [snake, camel] of Object.entries(currentModelFields)) {
            // Use word boundary for absolute precision
            const regex = new RegExp(`\\b${snake}\\b`, 'g');
            line = line.replace(regex, camel);
        }
    }
...
console.log('Final Field Normalization V5 Complete.');
