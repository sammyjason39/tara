...
for (let line of lines) {
    ...
    if (match) {
        const snake = match[2];
        const type = match[4].replace(/[\[\]\?]/g, '');
        if (snake.includes('_') && primitives.has(type)) {
...
for (const snake of sortedSnakes) {
    ...
    // Match line-start + indent + snake + space + PrimitiveType
...
console.log('Total System Normalization Complete.');
